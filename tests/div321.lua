test_data = {    
    ...
    [25] = {val1 = 22378921, val2 = 17854},
    [26] = {val1 = 3532891241, val2 = 2876541291},
    [27] = {val1 = 22441239, val2 = 67422},
    [28] = {val2 = 3532891241, val1 = 2876541291},
    [29] = {val1 = 3532891241, val2 = 1},
    [30] = {val1 = 2^28 + 16771, val2 = 2^24 + 69788}
}
...
function set_dword(val, addr)
    write_byte(addr+1, val % 256)
    
    val = math.floor(val / 256)
    write_byte(addr+2, val % 256)

    val = math.floor(val / 256)
    write_byte(addr+3, val % 256)

    val = math.floor(val / 256)
    write_byte(addr+4, val % 256)
end

function get_dword(addr)
    res = read_byte(addr+4)

    res = res * 256
    res = res + read_byte(addr+3)

    res = res * 256
    res = res + read_byte(addr+2)

    res = res * 256
    res = res + read_byte(addr+1)
...
function arrange()
    ...
    set_pc(load_address)
    set_dword(test_data[iter_count].val1, load_address + 3 + 5)
    set_dword(test_data[iter_count].val2, load_address + 3)
end
...
function assert()
    ...
    calc_mod = get_dword(load_address + 3 + 3 * 5)
    calc_div = get_dword(load_address + 3 + 2 * 5)
